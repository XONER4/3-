...
@admin_router.message(AdminState.waiting_for_user_id_balance)
...
@admin_router.message(AdminState.waiting_for_amount_balance)
...
@admin_router.message(AdminState.waiting_for_user_id_rank)
...
@admin_router.message(AdminState.waiting_for_rank)
...
@admin_router.message(AdminState.waiting_for_user_id_rename)
...
@admin_router.message(AdminState.waiting_for_new_name)
async def admin_rename_set(message: Message, state: FSMContext, session: AsyncSession):
    ...
    await session.commit()
    await message.answer(f"✅ Имя пользователя изменено на {user.full_name}.", reply_markup=admin_panel_keyboard())
    await notify_user(target_id, f"✏️ Администратор изменил ваше имя на {user.full_name}.")
    await state.clear()
...
@admin_router.message(AdminState.waiting_for_new_password)
async def admin_password_set(message: Message, state: FSMContext):
    global BOT_PASSWORD
    BOT_PASSWORD = message.text.strip()
    # Также можно сохранить в файл config.py, но для простоты обновим глобально
    import config
